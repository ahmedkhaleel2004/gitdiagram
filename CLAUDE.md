...
- **Early access** (`audience.ts`): anyone, on any device, in California, Washington, New York, Ontario, British Columbia, or within 60 km of London or Paris (Paris also matches all of Île-de-France) (Vercel IP geolocation headers) may start new videos; everyone can watch. It is an access rule, not a security boundary (VPNs pass). `VIDEO_PREVIEW_PAUSED=audience|device|limit` previews the paused UI in development. The operator can widen this to all desktops (phones outside those places stay out) or everyone, pause new videos, and override the daily limits live from `/admin` (see below).
- **Budget** (`limits.ts`): public generations are capped per UTC day overall, per person (one browser, a random-id cookie from `visitor.ts`; default 1) and per connection as a looser backstop (default 10, so a shared office still works) in Redis (fails closed), stop when ElevenLabs credits run low, and are locked per repo across instances. Only `VIDEO_ADMIN_TOKEN` (Bearer) may regenerate an existing video; locally every caller is trusted.
