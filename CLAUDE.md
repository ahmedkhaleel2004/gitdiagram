...
- **Early access** (`audience.ts`): anyone, on any device, in California, Washington, New York, Ontario, British Columbia or within 60 km of London (Vercel IP geolocation headers) may start new videos; everyone can watch. It is an access rule, not a security boundary (VPNs pass). `VIDEO_PREVIEW_PAUSED=audience|device|limit` previews the paused UI in development. The operator can widen this to all desktops (phones outside those places stay out) or everyone, pause new videos, and override the daily limits live from `/admin` (see below).
