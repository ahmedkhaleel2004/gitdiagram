...
This fork can generate a diagram from a local folder instead of a GitHub URL. It can also call an installed AI CLI for generation, so you can use an existing Codex/Gemini CLI login instead of setting an OpenAI/OpenRouter API key.
...
`AI_CLI_COMMAND` is split into a command plus arguments, and GitDiagram writes the generated prompt to the command's stdin. The trailing `-` in the Codex example tells `codex exec` to read the prompt from stdin, avoiding Windows command-line length limits for large repositories.
